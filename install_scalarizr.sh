...
PACKAGE_TO_INSTALL="scalarizr-ec2"
...
function install() {
    ...
    if [[ $OS_NAME =~ ("Red Hat"|CentOS|OEL|Amazon) ]] ; then
        ...
        echo 'baseurl=http://repo.scalr.net/rpm/latest/rhel/$releasever/$basearch' >> $REPO_FILE
        ...
        echo "deb http://repo.scalr.net/apt-plain latest/" > /etc/apt/sources.list.d/scalr-latest.list
